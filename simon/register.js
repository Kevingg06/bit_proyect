...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function enviarDatos_empresa() {
        ...
        if (datosVacios(inputs) && validarDatos(nombre) && validarContraseña(contraseña1) && segundaContraseña(contraseña1, contraseña2)) {
            ...
            // Crear token JWT
            const secretKey = 'tu_clave_secreta';  // Utiliza la misma clave secreta que en el servidor
            const token_empresa = jwt.sign({ email: email }, secretKey);
            console.log('Token JWT para empresa:', token_empresa);
            // Almacenar el token en una cookie
            document.cookie = `authToken=${token_empresa}; path=/`;
    ...
    function enviarDatos_empleado() {
        console.log('Función enviarDatos_empresa iniciada');
        ...
        if (datosVacios(inputs) && validarDatos(inputs) && validarContraseña(contraseña1) && segundaContraseña(contraseña1, contraseña2)) {
            ...
            // Crear token JWT
            const secretKey = 'tu_clave_secreta';  // Utiliza la misma clave secreta que en el servidor
            const token_empleado = jwt.sign({ email: email }, secretKey);
            console.log('Token JWT para empleado:', token_empleado);

            // Almacenar el token en una cookie
            document.cookie = `authToken=${token_empleado}; path=/`;

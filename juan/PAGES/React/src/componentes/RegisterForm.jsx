...
router.post('/', async  (req, res) => {
    ...
    db.query(insertQuery, queryParams, (err, result) => {
        ...
        if (result && result.affectedRows > 0) {
            ...
            res.cookie('authToken', JSON.stringify({ token, userId: result.insertId, nombre: nombre }), { httpOnly: false }); 
            res.status(201).json({ message: 'Usuario registradosssssss exitosamente.', token, userId: result.insertId });
        } else {
